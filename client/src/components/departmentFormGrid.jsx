...
const DepartmentFormGrid = ({updateFormGridData, userReport}) => {
  const questions = [
    'JOB KNOWLEDGE',
    'QUALITY OF WORK',
    'PRODUCTIVITY',
  ...
  const [formData, setFormData] = useState(userReport.formGridData ? userReport.formGridData : Array.from({ length: 13 }, () => ({ selfRating: '', supervisorRating: '', average: '' })))
//   console.log(userReport.formGridData)
          ...
          {questions.map((question, index) => (
            <tr key={index} className='border-b border-black'>
              ...
              <td>
                <input
                  type="number"
                  value={userReport.formGridData[index].selfRating}
...
export default DepartmentFormGrid;